...
namespace DeadlyTradeOverhaul
{
    public partial class ControlForm : Form
    {
        ...
        */
        ...
        #region [[[[[ Child Forms ]]]]]
        // PRICE CHECKING
        static DeadlySearchForm frmDeadlySearchForm = null;
        public static bool bShownSearchResult = false;
        ...
        private event EventHandler<EventArgs<string>> ClipboardParsingEvent;
        private event EventHandler deadlyPriceCheckingEvent;
        ...
        private void GetCurrencyCalcDictionary()
        {
            try
            {
                if(CurrencyCalcDictionary!=null)
            ...
            catch (Exception ex)
            {
                ...
                frmMSG.lbMsg.Text = "Unexpected exception occurred while get NinjaData.\r\nPlease check your network and Try again~.";
        ...
        private void MouseWheelExt(object sender, MouseEventExtArgs e)
        {
            try
            {
                ...
                if (InteropCommon.IsCtrlPressed())
                {
                    ...
                    if (ptCur.X >= int.Parse(LauncherForm.g_strGridLeft) && ptCur.X <= LauncherForm.g_nGridWidth)
                        return;
                    else
                    {
                        if (e.Delta > 0)
        ...
        private void _keymouseHooks_KeyDown(object sender, WindowsHook.KeyEventArgs e)
        {
            ...
            if (!LauncherForm.g_FocusLosing) // Only Exile Focused.
            {
                ...
                // CTRL + V
                if (e.Modifiers == WindowsHook.Keys.Control && e.KeyCode == WindowsHook.Keys.V && !bIsSettingsPop)
                {
                        ...
                        if (LauncherForm.g_POELogFileName == "KakaoClient.txt") // CTRL+V in KAKAO Client.
                        {
                            ...
                            if (m_strClipboardText != null && m_strClipboardText.Length > 0)
                            {
                                ...
                                if (m_strClipboardText.ToUpper().Contains("BUY YOUR") || m_strClipboardText.ToUpper().Contains("구매하고") || m_strClipboardText.ToUpper().Contains("WTB"))
                                {
                                    ...
                                    ClipboardParsingEvent(this, new EventArgs<string>("KAKAO BUYING"));
                ...
                /*else if (e.Modifiers == WindowsHook.Keys.Control && e.KeyCode == WindowsHook.Keys.D && !bIsSettingsPop && !LauncherForm.g_FocusLosing && !LauncherForm.g_FocusOnAddon)
                {
                     #region [[[[ Deadly Price Checking Event ]]]]
                    try
                    {
                        InteropCommon.SetForegroundWindow(LauncherForm.g_handlePathOfExile);
                        InputSimulator iSim = new InputSimulator();
                        ...
                        m_strClipboardText = ClipboardHelper.GetUnicodeText();
                        if (!String.IsNullOrEmpty(m_strClipboardText))
                        {
                            ClipboardParsingEvent(this, new EventArgs<string>("CTRL+D"));
                            InteropCommon.SetForegroundWindow(LauncherForm.g_handlePathOfExile);
                        }
                    }
                    catch (InvalidOperationException ex)
                ...
                #region ⨌⨌ Flask Timer ⨌⨌
                // FLASK 1
                if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Flask1.ToString()) && LauncherForm.g_bToggle1 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // FLASK 2
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Flask2.ToString()) && LauncherForm.g_bToggle2 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // FLASK 3
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Flask3.ToString()) && LauncherForm.g_bToggle3 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // FLASK 4
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Flask4.ToString()) && LauncherForm.g_bToggle4 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // FLASK 5
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Flask5.ToString()) && LauncherForm.g_bToggle5 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                #region ⨌⨌ Skill Timer ⨌⨌
                // Skill Q
                if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Skill1.ToString()) && LauncherForm.g_bToggleSkill1 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // Skill W
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Skill2.ToString()) && LauncherForm.g_bToggleSkill2 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // Skill E
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Skill3.ToString()) && LauncherForm.g_bToggleSkill3 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // Skill R
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Skill4.ToString()) && LauncherForm.g_bToggleSkill4 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
                ...
                // Skill T
                else if (e.KeyCode == (WindowsHook.Keys)Enum.Parse(typeof(WindowsHook.Keys), LauncherForm.g_Skill5.ToString()) && LauncherForm.g_bToggleSkill5 && !LauncherForm.g_FocusOnAddon && !bIsSettingsPop)
                {
                    try
                    {
        ...
        private void ControlForm_ClipboardParsingEvent(object sender, EventArgs<string> e)
        {
            if (e.Value == "KAKAO BUYING")
            {
                // if contain buying message.
                Parse_ClipboardBuying();
            }
            else if (e.Value == "CTRL+D")
            {
                // some cases...
                Parse_InGameCopyText();
            }
        }
        ...
        private void Parse_InGameCopyText()
        {
            //? is Map?
            // Y (ENG) contains ? "Travel to this Map by using it in a personal Map Device. Maps can only be used once."
            // Y (KOR) contains ? "Travel to this Map by using it in a personal Map Device. Maps can only be used once."

            // 2020.07.27 Restore Previous Feature that Check Map Mode (Words Include/Exclude :: Separator ;)
            InputSimulator iSim = new InputSimulator();

            iSim.Keyboard.ModifiedKeyStroke(VirtualKeyCode.CONTROL, VirtualKeyCode.VK_C);
            DeadlyPriceAPI.New_ItemTextParser(m_strClipboardText);
            ShowHidePriceResultForm();
        }

        private static void ShowHidePriceResultForm()
        {
            if (bShownSearchResult)
            {
                bShownSearchResult = false;
                frmDeadlySearchForm.Close();
            }
            else
            {
                bShownSearchResult = true;
                frmDeadlySearchForm = new DeadlySearchForm();
                frmDeadlySearchForm.Show();
            }
        }
        ...
        public bool Validate_POELogFile()
        {
            ...
            */
            try
            {
                ...
                {
                    MSGForm frmMSG = new MSGForm();
                    frmMSG.lbMsg.Text = "Can't read Add-on configuration.\r\nFile seems to be corrupt or delete.";
                    frmMSG.ShowDialog();
                    bRet = false;
            ...
            {
                MSGForm frmMSG = new MSGForm();
                frmMSG.lbMsg.Text = "Unknown Error occurred while Validate ini and log file path.\r\n\r\nERROR : ( " + ex.Message + " )";
                frmMSG.ShowDialog();
                bRet = false;
